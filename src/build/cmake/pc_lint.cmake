...
set(PC_LINT_EXECUTABLE "${SRC_ROOT}/scripts/flexelint/flint" CACHE STRING "full path to the pc-lint executable. NOT the generated lin.bat")
set(PC_LINT_CONFIG_DIR "${SRC_ROOT}/scripts/flexelint/" CACHE STRING "full path to the directory containing pc-lint configuration files")
...
execute_process(
	COMMAND bash "-c" "echo -n `arm-none-eabi-gcc --print-file-name=libgcc.a` | sed 's/libgcc.a/include/'"
	OUTPUT_VARIABLE
	GCC_LIB_DIR)
...
function(add_pc_lint target)
	...
	list(APPEND lint_include_directories_transformed -i"${GCC_LIB_DIR}")

	list(APPEND lint_include_directories_transformed -i"${CMAKE_BINARY_DIR}")
	...
	# Set modules setting
	if ((${target} MATCHES "ncl"))
	list(APPEND lint_module_customization -header "${SRC_ROOT}/ncl/ncl_header.pch" -i "${PC_LINT_CONFIG_DIR}" ncl.lnt)
	endif()

	if ((${target} MATCHES "rtos"))
		list(APPEND lint_module_customization -i "${PC_LINT_CONFIG_DIR}" rtos.lnt)
	endif()
